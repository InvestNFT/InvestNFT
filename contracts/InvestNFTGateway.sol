...
import "./BaseGateway.sol";

contract InvestNFTGateway is BaseGateway {
    ...
    function setNftData(address _nft, address _poolA, address _poolB, address _poolC, bool _increaseable, uint256 _delta) external onlyOwner override {
    ...
    function redeem(address _nft, uint256 _tokenId, uint256 _amount) external override nonReentrant {
        bytes32 infoHash = keccak256(abi.encodePacked(_nft, _tokenId));
        address fomulaA =  contractInfo[_nft].poolAddressA;
        ...
        uint256 totalBalanceA = poolsWeights[fomulaA]; // with hotpot, this value will be the sum of deposit and profit.
        uint256 tokenBalanceA = tokenInfo[infoHash].weightsFomulaA * totalBalanceA / poolsWeights[fomulaA];
