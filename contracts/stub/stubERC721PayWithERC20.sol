...
contract stubERC721PayWithERC20 is Ownable, ERC721Enumerable {
    ...
    function withdraw(address _address, uint256 _amount) external onlyOwner {
        ...
        payERC20.transfer(_address, _amount);
    ...
    function _mint(uint256 id) internal {
        gateway.depositWithERC20(id, 1, address(payERC20), FREE_MINTED_PRICE);
    ...
    function publicMint() hasInitialized canMint(1) external {

        uint256 amount = price;
        payERC20.transferFrom(msg.sender, address(this), amount);
        uint256 investPrice = amount * BASE_VALUE_PERCENTAGE / 100;
        payERC20.transfer(address(gateway), investPrice);
        uint256 id = totalSupply() + 1;
        mint(id);
        gateway.depositWithERC20(id, 1, address(payERC20), investPrice);
    }
    function freeMint() hasInitialized canMint(1) external {
        uint256 amount = price;
        uint256 investPrice = amount * BASE_VALUE_PERCENTAGE / 100;
        uint256 id = totalSupply() + 1;
        mint(id);
    ...
    function batchPublicMint(uint256 _numberTokens) hasInitialized canMint(_numberTokens) external {
